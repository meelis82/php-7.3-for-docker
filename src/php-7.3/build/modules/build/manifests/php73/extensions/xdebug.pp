class build::php73::extensions::xdebug {
  ...
  bash_exec { 'cd /tmp/xdebug-2.7.2 && phpize-7.3.8':
    require => Bash_exec['cd /tmp && tar xzf xdebug-2.7.2.tgz']
  }

  bash_exec { 'cd /tmp/xdebug-2.7.2 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.8':
    timeout => 0,
    require => Bash_exec['cd /tmp/xdebug-2.7.2 && phpize-7.3.8']
  }

  bash_exec { 'cd /tmp/xdebug-2.7.2 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/xdebug-2.7.2 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.8']
  }
