class build::php71::extensions::memcached {
  require build::php71
  ...
  file { '/tmp/php-memcached-php7.zip':
    ...
    source => 'puppet:///modules/build/tmp/php71/extensions/memcached/php-memcached-php7.zip',
  ...
  bash_exec { 'cd /tmp/php-memcached-php7 && phpize-7.1.26':
    require => Bash_exec['cd /tmp && unzip php-memcached-php7.zip']
  }

  bash_exec { 'cd /tmp/php-memcached-php7 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.1.26 --enable-memcached-igbinary':
    timeout => 0,
    require => Bash_exec['cd /tmp/php-memcached-php7 && phpize-7.1.26']
  }

  bash_exec { 'cd /tmp/php-memcached-php7 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/php-memcached-php7 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.1.26 --enable-memcached-igbinary']
  }
