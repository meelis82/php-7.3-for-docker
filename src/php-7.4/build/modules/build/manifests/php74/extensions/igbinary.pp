class build::php74::extensions::igbinary {
  require build::php74

  file { '/tmp/igbinary7-master.zip':
    ensure => present,
    source => 'puppet:///modules/build/tmp/php74/extensions/igbinary/igbinary7-master.zip'
  }
  ...
  bash_exec { 'cd /tmp/igbinary7-master && phpize-7.4.2':
    require => Bash_exec['cd /tmp && unzip igbinary7-master.zip']
  }

  bash_exec { 'cd /tmp/igbinary7-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.4.2 --enable-igbinary':
    timeout => 0,
    require => Bash_exec['cd /tmp/igbinary7-master && phpize-7.4.2']
  }

  bash_exec { 'cd /tmp/igbinary7-master && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/igbinary7-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.4.2 --enable-igbinary']
  }
